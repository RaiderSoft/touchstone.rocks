...
SettingsAction DrawGameSettings(GameSettings& settings, int& selected_preset,
                                bool chat_consumed, int vision_board_size) {
  ...
  // White button.
  {
    ...
    Color bg = selected ? Color{60, 60, 60, 255} : Color{50, 50, 50, 255};
  ...
  // --- Board size ---
  bool size_locked = vision_board_size > 0;
  if (size_locked) {
    settings.board_size = vision_board_size;
    settings.komi = DefaultKomi(vision_board_size);
  }
  DrawText("Board size:", cx - 170, y, 22, RAYWHITE);
  if (size_locked)
    DrawText("(vision)", cx + 10, y, 18, Color{120, 180, 120, 255});
  y += 36;

  {
    ...
    for (int i = 0; i < NUM_SIZES; i++) {
      ...
      Rectangle btn = {(float)bx, (float)y, (float)SIZE_BTN_W,
                        (float)SIZE_BTN_H};
      bool hover = !size_locked && CheckCollisionPointRec(mouse, btn);
      bool sel = (settings.board_size == kSizes[i]);
      Color bg = sel   ? Color{40, 60, 40, 255}
                 : size_locked ? Color{38, 38, 38, 255}
                               : Color{50, 50, 50, 255};
      if (hover && !sel) bg = Color{60, 60, 60, 255};
      DrawRectangleRec(btn, bg);
      Color border =
          sel ? Color{100, 200, 100, 255}
              : size_locked ? Color{55, 55, 55, 255} : Color{80, 80, 80, 255};
      DrawRectangleLinesEx(btn, sel ? 2.0f : 1.0f, border);
      int ltw = MeasureText(kSizeLabels[i], 18);
      Color text_col = sel   ? Color{180, 255, 180, 255}
                       : size_locked ? Color{100, 100, 100, 255}
                                     : RAYWHITE;
...

  {
    ...
    int total_w = KOMI_BTN_SZ * 2 + KOMI_GAP * 2 + 60;
    int kx = cx - total_w / 2;
  ...
  SettingsAction result = SettingsAction::kNone;
